  ...
  for (let i = 0; i < sentenceLength; i++) {
    particles.push(
      new Particle(
        ...
        sentence[(i * 2) % sentence.length],
        sentence[(i * 2 + 1) % sentence.length],
        i
      )
    );
  }
...
function draw() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    let mouseAttraction = p5.Vector.sub(
...
KDTree.prototype.insert = function (particle) {
  this.root = KDNode.prototype.insert(
    this.root,
    particle,
    0,
    0,
...
KDNode.prototype.draw = function (node, drawVert) {
  ...
  if (drawVert == true) {
    ...
    if (node.right_up == null) {
      ...
      scale(
        rectWidth / (textWidth(node.particle.leftTextContent) + 10),
        (rectHeight - 5) / textSizeValue
      );
    ...
    if (node.left_down == null) {
      ...
      translate(rectX, rectY);
      scale(
        rectWidth / (textWidth(node.particle.rightTextContent) + 10),
        (rectHeight - 5) / textSizeValue
      );
      fill(node.particle.textColor);
...
KDNode.prototype.search = function (
  /*KDNode*/ node,
  /*Vector*/ p,
  ...
  if (node == null) {
    ...
    node.particle.position.x == p.x &&
    node.particle.position.y == p.y
  ) {
    print("same!");
    return node;
  }
...
KDNode.prototype.insert = function (
  /*KDNode*/ node,
  /*Particle*/ p,
  /*float*/ x0,
  /*float*/ y0,
  ...
  if (node == null) {
    ...
    node.particle.position.x == p.position.x &&
  ...
  if (xcmp == 1) {
    ...
    if (cmp < 0)
      node.left_down = node.insert(
        node.left_down,
        p,
        x0,
        y0,
        node.particle.position.x,
        y1,
        xcmp
      );
    else
      node.right_up = node.insert(
        node.right_up,
        p,
        node.particle.position.x,
        y0,
