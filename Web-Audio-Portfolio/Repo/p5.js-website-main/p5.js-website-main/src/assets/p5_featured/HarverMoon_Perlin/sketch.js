...
function draw() {
  ...
  for (var i = 0; i < particles.length; i++) {
    ...
    particles[i].draw();
    var noiseRot = map(
      noise(particles[i].nowPos.x * 0.006, particles[i].nowPos.y * 0.006),
      0.2,
      0.8,
      0,
      PI * 2
  ...
  for (var j = 0; j < particles.length; j++) {
    if (
      particles[j].nowPos.x >= width - 10 ||
      particles[j].nowPos.x <= 10 ||
      particles[j].nowPos.y >= height - 10 ||
      particles[j].nowPos.y <= 10 ||
      particles[j].lifeCount <= 0
    ) {
