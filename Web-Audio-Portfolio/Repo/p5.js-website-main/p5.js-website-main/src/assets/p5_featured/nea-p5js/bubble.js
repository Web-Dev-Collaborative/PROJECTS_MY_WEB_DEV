function Bubble(d, xp, yp, xs, ys, eText, textw, texth, txx, tyy) {
  ...
  this.collide = function (m, n) {
    //m is each of the mover objects
    //if dist between centers of movers is less than both their radius added -> colliding!
    if (
      dist(m.position.x, m.position.y, this.position.x, this.position.y) <
        m.mass / 2 + this.mass / 2 ||
      dist(n.position.x, n.position.y, this.position.x, this.position.y) <
        n.mass / 2 + this.mass / 2
    ) {
      if (this.bang == false) {
