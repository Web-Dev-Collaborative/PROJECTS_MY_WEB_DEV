(function (root) {
  ...
  mainModule(function () {
    ...
    Tone._defineGetterSetter = function (constr, property) {
      ...
      if (
        proto.hasOwnProperty(getterName) &&
  ...
  toneModule(function (Tone) {
    ...
    Tone.Compressor = function () {
       ...
       *  @type {Tone.Signal}
       */
      this.attack = new Tone.Signal(
        this._compressor.attack,
        Tone.Signal.Units.Time
      );
  ...
  toneModule(function (Tone) {
    ...
    Tone.LowpassCombFilter = function () {
      ...
      // buffersize / number of samples = number of delays needed per buffer frame
      var delayCount = Math.ceil(
        this.bufferSize / (options.minDelay * this.context.sampleRate)
      );
      //set some ranges
      delayCount = Math.min(delayCount, 10);
      delayCount = Math.max(delayCount, 1);
     ...
     */
    Tone.LowpassCombFilter.prototype.setDelayTimeAtTime = function (
      delayAmount,
      time
    ) {
  ...
  toneModule(function (Tone) {
    ...
    Tone.Recorder = function (channels) {
      console.warn(
        "Tone.Recorder is deprecated. It will be removed in next version"
      );
  ...
  toneModule(function (Tone) {
    ...
    Tone.DuoSynth = function (options) {
      ...
      this.vibratoAmount = new Tone.Signal(
        this._vibratoGain.gain,
        Tone.Signal.Units.Gain
      );
  ...
  toneModule(function (Tone) {
    ...
    Tone.MultiSampler = function (samples, onload) {
      console.warn(
        "Tone.MultiSampler is deprecated - use Tone.PolySynth with Tone.Sampler as the voice"
      );
      Tone.Instrument.call(this);
