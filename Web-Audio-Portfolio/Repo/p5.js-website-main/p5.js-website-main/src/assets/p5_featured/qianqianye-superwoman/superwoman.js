...
function SuperWoman(startPos, inColor) {
  ...
  this.superwomanStart = function () {
    ...
    for (var i = 0; i < 3; i++) {
      var randomPosition = createVector(
        3 * random(0, width),
        3 * random(0, this.posWait.y - 300)
      ); //FLY PATH
  ...
  this.stateMachine = function () {
    if (this.state == 0) {
      ...
      //fly state
      this.flyTime = this.flyTime + 0.01;
      if (this.flyTime >= 1) {
        this.state = 1;
        this.upDown = false;
        this.flyTime = 0;
      ...
      for (var i = 0; i <= this.flyTime; i += 0.01) {
        // FLYTRACE
        var traceX = bezierPoint(
          this.posFlyto[0].x,
          this.posFlyto[1].x,
          this.posFlyto[2].x,
          this.posFlyto[0].x,
        ...
        ellipse(
          ...
          (i / this.flyTime) * 10,
          (i / this.flyTime) * 10
        ); // Trace weight changes by flytime
      }
    }
  };
}
