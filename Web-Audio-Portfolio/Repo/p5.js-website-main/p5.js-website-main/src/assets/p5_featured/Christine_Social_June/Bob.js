...
function Bob() {
  ...
  bobView.runInterference = function (bill, index, bills) {
    ...
    if (distance > 0) {
      ...
      for (
        var i = fieldPulseFrame;
        i < thisBob.fieldRadius;
        i += thisBob.fieldIncrement
      ) {
        //for each ring of otherBob's field
        for (
          var j = fieldPulseFrame;
          j < otherBob.fieldRadius;
          j += otherBob.fieldIncrement
        ) {
  ...
  bobView.renderIntersectShape = function (
    intersections,
    distance,
    otherHue,
    i
  ...
  bobView.renderField = function () {
    ...
    for (
      var i = fieldPulseFrame;
      i < bobView.fieldRadius;
      i += bobView.fieldIncrement
    ) {
  ...
  bobView.driveBob = function () {
    ...
    if (keyIsDown(LEFT_ARROW)) driveForce.add(-1 * driveForceIncrement, 0);

    if (keyIsDown(RIGHT_ARROW)) driveForce.add(driveForceIncrement, 0);

    if (keyIsDown(UP_ARROW)) driveForce.add(0, -1 * driveForceIncrement);

    if (keyIsDown(DOWN_ARROW)) driveForce.add(0, driveForceIncrement);
