...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    exec: {
      build_examples:
        "node <%= config.src %>/data/examples/build_examples/build.js <%= config.src %>/templates/pages/examples/",
      build_libraries:
        "node <%= config.src %>/data/libraries/build.js <%= config.src %>/templates/pages/libraries/",
      build_learn:
        "node <%= config.src %>/data/learn/build.js <%= config.src %>/templates/pages/learn/",
    },
    watch: {
      assemble: {
        files:
    ...
    assemble: {
      pages: {
        options: {
          ...
          plugins: ["assemble-contrib-permalinks", "assemble-contrib-i18n"],
          i18n: {
          ...
          permalinks: {
            ...
            patterns: [
              ...
              },
              {
                pattern: ":base",
                replacement: function () {
                  var check = this.basename.lastIndexOf(
    ...
    requirejs: {
      yuidoc_theme: {
        options: {
          ...
          mainConfigFile:
            "<%= config.src %>/yuidoc-p5-theme-src/scripts/config.js",
          name: "main",
  ...
  grunt.registerTask("update-version", function () {
    ...
    const version = require("./src/templates/pages/reference/data.json").project
      .version;
  ...
  grunt.registerTask("run", ["connect:livereload", "watch"]);

  // optimize assets
  grunt.registerTask("optimize", ["newer:imagemin", "concat:dist", "postcss"]);
