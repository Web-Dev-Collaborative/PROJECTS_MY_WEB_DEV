...
export default class Matrix {
  constructor(rows, columns) {
    ...
    this.rotate = {
      ...
      all: (amount) => {
        ...
        for (let i = 0; i < this.rows; i++) {
          let cut = this.pattern[i].splice(
            this.pattern[i].length - amount,
            amount
          );
      ...
      row: (row, amount) => {
        ...
        let cut = this.pattern[row].splice(
          this.pattern[row].length - amount,
          amount
        );
  ...
  formatAsText() {
    ...
    this.iterate(
      (r, c) => {
        patternString += (this.pattern[r][c] ? 1 : 0) + " ";
      },
      () => {
        patternString += "\n";
      }
