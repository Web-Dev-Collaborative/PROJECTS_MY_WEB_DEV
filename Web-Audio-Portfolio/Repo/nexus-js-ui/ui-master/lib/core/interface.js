...
export default class Interface extends EventEmitter {
  ...
  attachListeners() {
    ...
    // Setup interaction
    if (touch.exists) {
      this.interactionTarget.addEventListener("touchstart", (evt) =>
        this.preTouch(evt)
      );
      this.interactionTarget.addEventListener("touchmove", (evt) =>
        this.preTouchMove(evt)
      );
      this.interactionTarget.addEventListener("touchend", (evt) =>
        this.preTouchRelease(evt)
      );
    }
    this.boundPreMove = (evt) => this.preMove(evt);
    this.boundPreRelease = (evt) => this.preRelease(evt);
    this.interactionTarget.addEventListener("mousedown", (evt) =>
      this.preClick(evt)
    );
  }
