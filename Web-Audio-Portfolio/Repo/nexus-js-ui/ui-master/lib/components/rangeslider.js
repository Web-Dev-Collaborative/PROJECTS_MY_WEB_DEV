...
export default class RangeSlider extends Interface {
  constructor() {
    ...
    if (this.mode === "drag") {
      this.position = {
        center: new Interaction.Handle(
          "relative",
          "horizontal",
          [0, this.width],
          [this.height, 0]
        ),
        size: new Interaction.Handle(
          "relative",
          "vertical",
          [0, this.width],
          [this.height, 0]
      ...
      this.position = {
        ...
        start: new Interaction.Handle(
          "relative",
          "horizontal",
          [0, this.width],
          [this.height, 0]
        ),
        end: new Interaction.Handle(
          "relative",
          "horizontal",
          [0, this.width],
          [this.height, 0]
  ...
  resize(w, h) {
    ...
    this.bar.setAttribute(
      "width",
      (this.range.end.normalized - this.range.start.normalized) * this.width
    );
  ...
  render() {
    this.ref.setAttribute(
      "transform",
      "translate(" + this.range.start.normalized * this.width + ", 0)"
    );
    this.bar.setAttribute(
      "width",
      (this.range.end.normalized - this.range.start.normalized) * this.width
  ...
  move() {
    if (this.clicked) {
      ...
      if (this.mode === "drag") {
        ...
        this.range.center = math.scale(
          this.position.center.value,
          0,
          1,
          this.min,
          this.max
