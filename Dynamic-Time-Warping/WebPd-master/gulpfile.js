...
gulp.task("lib.concat", function () {
  return gulp
    .src(["./deps/AudioContextMonkeyPatch.js", "./build/webpd-latest.js"])
...
gulp.task("lib.objectList", function () {
  ...
  rendered = mustache.render("{{#objects}}- {{{.}}}\n{{/objects}}", {
    objects: Object.keys(library).sort(),
  });

  return file("OBJECTLIST.md", rendered, { src: true }).pipe(gulp.dest("."));
});
...
gulp.task("test.browser.build", function (done) {
  ...
  browserTestWatcher = gulp.watch(
    ["./test/**/*.js", "waatest/lib-build.js"],
