...
exports.declareObjects = function (library) {
  ...
  library["int"] = library["i"] = _NumberBase.extend({
    ...
    setVal: function (val) {
      if (!_.isNumber(val)) return console.error("invalid [int] value " + val);
    ...
    inletDefs: [
      portlets.Inlet.extend({
        message: function (args) {
          ...
          else if (val !== "bang") console.error("invalid message : " + args);
  ...
  library["dbtorms"] = _OneNumVarFunctionBase.extend({
    type: "dbtorms",
    compute: function (inVal) {
      return inVal <= 0 ? 0 : Math.exp((Math.LN10 * (inVal - 100)) / 20);
    },
  });
  ...
  library["dbtopow"] = _OneNumVarFunctionBase.extend({
    type: "dbtopow",
    compute: function (inVal) {
      return inVal <= 0 ? 0 : Math.exp((Math.LN10 * (inVal - 100)) / 10);
    },
  });
      ...
      portlets.Inlet.extend({
        message: function (args) {
          ...
          if ((ind = this.obj.filters.indexOf(msg)) !== -1)
            ...
            this.obj.outlets.slice(-1)[0].message(utils.timeTag([msg], args));
        },
      }),
  ...
  library["swap"] = PdObject.extend({
    ...
    setVal: function (val) {
      if (!_.isNumber(val)) return console.error("invalid [swap] value " + val);
      this.val = val;
    },
  });
        ...
        message: function (args) {
          ...
          if (msg === "bang")
            this.obj
              .o(0)
              .message(
                utils.timeTag([Math.floor(Math.random() * this.obj.max)], args)
              );
      ...
      portlets.Inlet.extend({
        message: function (args) {
          ...
          this.obj.outlets[0].message(
            utils.timeTag([utils.getTimeTag(args) - this.obj.refTime], args)
          );
        },
      }),
    ],
  ...
  library["array"] = library["table"] = PdObject.extend(
    ...
    mixins.EventEmitterMixin,
    {
      ...
      setData: function (audioData, resize) {
        ...
        this.data.set(
          audioData.subarray(0, Math.min(this.data.length, audioData.length))
        );
          ...
          if (command === "read") {
            ...
            pdGlob.storage.get(url, function (err, arrayBuffer) {
              ...
              pdGlob.audio.decode(arrayBuffer, function (err, audioData) {
                ...
                self.obj
                  .o(0)
                  .message([Math.min(arrays[0].size, audioData[0].length)]);
              });
            });
