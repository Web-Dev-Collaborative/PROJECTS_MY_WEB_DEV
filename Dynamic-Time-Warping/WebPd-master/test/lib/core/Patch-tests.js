...
describe("core.Patch", function () {
  var PortletMixin = {
    ...
    connection: function () {
      this.connectionCalled++;
    },
    disconnection: function () {
      this.disconnectionCalled++;
    },
  };
  ...
  describe(".start", function () {
      ...
      pdGlob.library["spy"] = PdObject.extend({
        outletDefs: [
          portlets.Outlet.extend({
            start: function () {
              called.push(this.obj.spyId + ":o(0).start");
            },
          }),
        ],
        init: function (args) {
    ...
    it("should call start on subpatches as well", function () {
      var patch = new Patch(),
        ...
        subpatchStarted = 0,
        subsubpatchStarted = 0;

      subpatch.on("started", function () {
        subpatchStarted++;
      });
      subsubpatch.on("started", function () {
        subsubpatchStarted++;
      });
      patch.start();
  ...
  describe(".createObject", function () {
    ...
    it("should throw UnknownObjectError if object unknown", function () {
      ...
      assert.throws(function () {
        patch.createObject("Idonotexist", []);
  ...
  describe(".resolveArgs", function () {
    it("should resolve $-args", function () {
      ...
      assert.deepEqual(patch.resolveArgs([123, "$0", "$1", 456, "$2", "$3"]), [
        123,
        9999,
        11,
        456,
