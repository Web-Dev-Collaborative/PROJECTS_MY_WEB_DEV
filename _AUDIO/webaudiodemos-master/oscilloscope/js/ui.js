...
function draw() {
  if (myOscilloscope) {
    ...
    if (freqCanvas) drawFreqBars(myOscilloscope.analyser, freqCanvas.context);
  }
...
function setupCanvases(container) {
  ...
  if (container) container.appendChild(scopeCanvas);
  else document.body.appendChild(scopeCanvas);
  ...
  if (container) container.appendChild(freqCanvas);
  else document.body.appendChild(freqCanvas);
}
...
function dutycyclechange() {
  pwmOsc.setDutyCycle(
    1 - parseFloat(document.getElementById("dutycycle").value)
  );
}
