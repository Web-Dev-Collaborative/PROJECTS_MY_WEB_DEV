function midiMessageReceived(ev) {
  ...
  if (cmd == 8 || (cmd == 9 && velocity == 0)) {
    ...
    // note off
    if (channel != 9) noteOff(noteNumber);
  } else if (cmd == 9) {
    // note on
    console.log(
      "Note on - channel: " +
    ...
    if (channel == 9 || channel == 1) playDrum(noteNumber, velocity);
    else noteOn(noteNumber, velocity);
  } else if (cmd == 11) {
    console.log(
      "controller: channel: " +
        channel +
...
function changeMIDIOut(ev) {
  ...
  if (typeof midiAccess.outputs == "function")
    //Old Skool MIDI outputs() code
    midiOut = midiAccess.outputs()[ev.target.selectedIndex];
  else midiOut = midiAccess.outputs.get(id);
  ...
  if (typeof midiAccess.inputs == "function") {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (!midiIn && str.indexOf("Controls") != -1) {
        selectMIDIIn.appendChild(
          new Option(list[i].name, list[i].id, true, true)
        );
        midiIn = list[i];
      } else
        selectMIDIIn.appendChild(
          new Option(list[i].name, list[i].id, false, false)
        );
    }
    if (!midiIn) midiIn = list[0];
    if (midiIn) midiIn.onmidimessage = midiMessageReceived;
    ...
    if (!midiOut) midiOut = list[0];
  } else {
    // new MIDIMap implementation:
    ...
    for (
      ...
      selectMIDIOut.appendChild(
        new Option(output.name, output.id, preferred, preferred)
      );
      if (preferred) {
        midiOut = output;
        outputIsLivid = true;
...
function hideBeat(index) {
  if (midiOut && outputIsLivid) midiOut.send([0x80, 16 + index, 0x00]);
}
...
function setActiveInstrument(index) {
  ...
  for (var beat = 0; beat < 16; beat++) showCorrectNote(beat, notes[beat]);
}
...
function showCorrectNote(index, note) {
  ...
  if (midiOut && outputIsLivid) midiOut.send([0x90, 32 + index, note * 32]);
}
...
function noteOn(noteNumber, velocity) {
  ...
  } else if (noteNumber >= 32 && noteNumber < 48)
    // bottom row - beats
...
function controller(number, data) {
  switch (number) {
    ...
    case 48: // Filter cutoff
      if (!filterEngaged && data < 127) return;
      filterEngaged = true;
      setFilterCutoff(data / 127.0);
      // echo back out - this lights up the control
      if (midiOut && outputIsLivid) midiOut.send([11 << 4, 48, data]);
      return;
