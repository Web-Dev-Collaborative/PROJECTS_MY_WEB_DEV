...
<br><br><b><?php echo $spTextPanel["Current Time"]?>:</b> <?php echo date("Y-m-d H:i:s <b>T(P)</b>"); ?>
<?php echo $pagingDiv?>
<table width="100%" id="cust_tab">
	<tr class="listHead">
		<th><		<th><?php echo $spText['common']['Id']?></th>
		<th><?php echo $spText['common']['Category']?></th>	
		<th><?php echo $spText['label']['Subject']?></th>
		<th><?php echo $spText['common']['Details']?></th>
		<th><?p		<th><?php echo $spText['common']['Action']?></th>
	</tr>
	<?php
	$colCoun	if (count($list) > 0) {
		foreach ($list as $listInfo) {
		    $alertLink = scriptAJAXLinkHrefDialog($pgScriptPath, 'content', "sec=alert_info&id=".$listInfo['id'], $listInfo['id']);            
			?>
			<tr 				<td style="width: 20px;"><input type="checkbox" name="ids[]" value="<?php echo $listInfo['id']?>"></td>
				<td><?php echo $alertLink?></td>
				<td><?php echo $alertCategory[$listInfo['alert_category']]?></td>
				<td				<td><?php echo stripslashes($listInfo['alert_message'])?></td>
